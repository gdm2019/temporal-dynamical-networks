...
def egonet_kernel( dataname, eventname, root_data, loadflag, saveloc ):
	...
	savename = saveloc + 'egonet_kernel_' + eventname[:-4] + '.pkl'
	...
	if loadflag == 'y': #load files
		...
		names = ['nodei', 'nodej', 'tstamp'] #column names
		#load (unique) event list: node i, node j, timestamp
		filename = root_data + dataname + '/data_formatted/' + eventname[:-4] + '.evt'
		events = pd.read_csv( filename, sep=';', header=None, names=names )
		#reverse nodes i, j in all events
		events_rev = events.rename( columns={ 'nodei':'nodej', 'nodej':'nodei' } )[ names ]
		#duplicate events (all nodes with events appear as node i)
		...
		for pos, (nodei, events_nodei) in enumerate( events_concat.groupby('nodei') ): #loop through egos
			...
			## START NEW
			index = pd.MultiIndex.from_arrays([nodei * np.ones(len(act_counts), dtype=int), np.arange(len(act_counts), dtype=int)])
			new_series = pd.Series(act_counts / act_options.astype(float), index=index)
			egonet_kernel = pd.concat([egonet_kernel, new_series])
			## END NEW

		## OLD
		print(type(egonet_kernel.index))
...
def egonet_jaccard( eventname, loadflag, saveloc, nodei='nodei', nodej='nodej', saveflag=True ):
	...
	savename = saveloc + 'egonet_jaccard_' + eventname[:-4] + '.pkl'
	...
	if loadflag == 'y': #load file
		...
		egonet_acts_pieces = pd.read_pickle( saveloc + 'egonet_acts_pieces_' + eventname[:-4] + '.pkl' )
