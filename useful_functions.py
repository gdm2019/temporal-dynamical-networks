...
#         if width >= time_shift: 
#             number_windows = int(np.floor((tmax - tmin - width)/time_shift) + 1)
#             print('number_windows:', number_windows)

#             if number_windows >= 2: 
#                 max_w = min(number_windows, MAX_windows)
#                 activity = np.zeros((len(edges), max_w)) #{} # 2d array
#                 end_t = tmin
#                 for index in range(max_w):
...
#                             activity[index_edge, index] = 1
#                         else:
#                             activity[index_edge, index] = 0

#                 n_active = np.zeros((len(nodes), len(activity))) #{} # 2d array
#                 delta_n_active = {}
#                 delta_n_active_static = {}
#                 delta_n_active_mean_dynamic = {}
#                 mean_dynamic_degree = np.zeros(len(nodes)) #{} # 2d array
#                 mean_dynamic_degree_static = {}
#                 for index_ind, ind in enumerate(nodes): 
#                     lines = df[(df['sender'] == ind) | (df['recipient'] == ind)]
#                     aux = np.where(np.isin(np.array(list(edges)), list(lines['edge'].drop_duplicates())))[0] #list(lines['edge'].drop_duplicates())
#                     iter = 0
#                     for time_point in range(max_w):
#                         n_active[index_ind, time_point] = np.sum(activity[index_aux, time_point] for index_aux in aux) #np.sum(activity[aux, time_point] == 1)
#                         if iter > 0:
#                             delta_k = n_active[index_ind, time_point] - n_active[index_ind, time_point - 1]
#                             if n_active[index_ind, time_point - 1] not in delta_n_active:
#                                 delta_n_active[n_active[index_ind, time_point - 1]] = []
#                             delta_n_active[n_active[index_ind, time_point - 1]].append(delta_k)
...
#                     mean_dynamic_degree[index_ind] = np.mean([n_active[index_ind, time_point] for time_point in range(len(activity))])
#                     if agg_degree[ind] not in mean_dynamic_degree_static:
#                         mean_dynamic_degree_static[agg_degree[ind]] = []
#                     mean_dynamic_degree_static[agg_degree[ind]].append(np.mean([n_active[index_ind, time_point] for time_point in range(len(activity))]))
#                     iter = 0
#                     for time_point in range(max_w):
#                         n_active[index_ind, time_point] = np.sum(activity[index_aux, time_point] for index_aux in aux)
#                         if iter > 0:   
#                             delta_k = n_active[index_ind, time_point] - n_active[index_ind, time_point - 1]                 
#                             if mean_dynamic_degree[index_ind] not in delta_n_active_mean_dynamic:
#                                 delta_n_active_mean_dynamic[mean_dynamic_degree[index_ind]] = []
#                             delta_n_active_mean_dynamic[mean_dynamic_degree[index_ind]].append(delta_k)
#                         iter += 1
...
#                     ax1.axvline(x = np.mean(mean_dynamic_degree), linestyle = '--', color=chosen_color, label = f'mean dyn. degree')
#                     xlim = ax1.get_xlim()
#                     x_bisector1 = np.linspace(xlim[0], xlim[1], 100)
#                     y_bisector1 = -x_bisector1
...
#                         xlim = ax4.get_xlim()
#                         x_bisector4 = np.linspace(xlim[0], xlim[1], 100)
...
#         ax1.plot(x_bisector1, y_bisector1, color='black', label='y = -x')
...
#         ax3.plot(x_bisector3, y_bisector3, color='black', label='y = -x')
#         ax3.set_title(f'{dataset_name}', fontsize = 22)

#     if plot5 == True:
#         ax4.plot(x_bisector4, x_bisector4, color='black', label='y = x')
...
def get_spectrum(dataset_name, df, MAX_windows, widths_ref, time_shifts_ref, plot1, plot2, plot3, plot4, plot5):
    ...
    for iter_windows in range(len(widths)):
        ...
        if width >= time_shift: 
            ...
            if number_windows >= 2: 
                max_w = min(number_windows, MAX_windows)
                activity = np.zeros((len(edges), max_w))
                delta_activity = {}
                end_t = tmin
                for index in range(max_w):
                    start_t = tmin + time_shift*index
                    end_t = start_t + width
                    df_window = df[(df['timestamp'] >= start_t) & (df['timestamp'] < end_t)]
                    for index_e, edge in enumerate(edges):
                        if edge in list(df_window['edge'].drop_duplicates()):
                            activity[index_e, index] = 1
                        else:
                            activity[index_e, index] = 0
                ...
                for ind in nodes: 
                    ...
                    indices_aux = [index_e for index_e, value in enumerate(edges) for edge_aux in aux if value == edge_aux]
                    for iter, time_point in enumerate(range(max_w)):
                        n_active[ind, time_point] = np.sum([activity[index_e, time_point] == 1 for index_e in indices_aux])
                        if iter > 0:
                            delta_k = n_active[ind, time_point] - n_active[ind, time_point - 1]
                            if plot1 == True or plot2 == True:
                                if n_active[ind, time_point - 1] not in delta_n_active:
                                    delta_n_active[n_active[ind, time_point - 1]] = []
                                delta_n_active[n_active[ind, time_point - 1]].append(delta_k)
                            if plot3 == True: 
                                if agg_degree[ind] not in delta_n_active_static:
                                    delta_n_active_static[agg_degree[ind]] = []
                                delta_n_active_static[agg_degree[ind]].append(delta_k)

                    mean_dynamic_degree[ind] = np.mean([n_active[ind, time_point] for time_point in range(max_w)])
                    if plot5 == True:
                        if agg_degree[ind] not in mean_dynamic_degree_static:
                            mean_dynamic_degree_static[agg_degree[ind]] = []
                        mean_dynamic_degree_static[agg_degree[ind]].append(np.mean([n_active[ind, time_point] for time_point in range(max_w)]))
                    if plot4 == True:
                        for iter, time_point in enumerate(range(max_w)):
                            n_active[ind, time_point] = np.sum([activity[index_e, time_point] == 1 for index_e in indices_aux])
                            if iter > 0:   
